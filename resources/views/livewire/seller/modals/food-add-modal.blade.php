<div>
    <x-jet-dialog-modal wire:model="confirmingfoodAddModal">
        <x-slot name="title">
            <h2 class="font-medium text-base ml-auto">
                {{ __('افزودن غذای جدید')}}</h2>
        </x-slot>
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmingfoodAddModal', false)"
                                    class="btn btn-outline-secondary w-20 ml-1">
                {{ __('لغو') }}
            </x-jet-secondary-button>

            <x-jet-button class="ml-2 btn btn-primary w-20" wire:click="saveFood()">
                {{ __('ذخیره') }}
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
