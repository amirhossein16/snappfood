<div>
    <x-jet-dialog-modal wire:model="confirmingDiscountEditModal">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmingDiscountEditModal', false)"
                                    wire:loading.attr="disabled" class="btn btn-outline-secondary w-20 ml-1">
                {{ __('لغو') }}
            </x-jet-secondary-button>

            <x-jet-button class="ml-2 btn btn-primary w-20" wire:click="editDiscount()" wire:loading.attr="disabled">
                {{ __('ذخیره') }}
            </x-jet-button>
        </x-slot>
    </x-jet-dialog-modal>
